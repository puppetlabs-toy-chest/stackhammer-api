...
public class AccessTokenRetriever {
	static class GitHubApplication implements Serializable {
		private static final long serialVersionUID = 821882903346426108L;

		private final String clientId;

		private final String clientSecret;

		public GitHubApplication(String clientId, String clientSecret) {
	...
	private static void addPair(Map<String, String> result, String pair) throws UnsupportedEncodingException {
		...
		if(sep < 0)
			...
			result.put(
				URLDecoder.decode(pair.substring(0, sep), QUERY_ENCODING),
				URLDecoder.decode(pair.substring(sep + 1), QUERY_ENCODING));
	}
	...
	public AccessTokenRetriever(String user, String password, GitHubApplication app) {
		this.user = user;
	...
	public synchronized String getAccessToken(String... scopes) throws SecurityException, IOException {
		...
		url = new URL(
			OAUTH_URL +
					"access_token?client_id=" + app.getClientId() + "&client_secret=" + app.getClientSecret() + "&code=" + code); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
